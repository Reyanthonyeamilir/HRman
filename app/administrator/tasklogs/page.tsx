...
export default function TaskLogsPage() {
        ...
        <div class="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
          <div class="border-b border-gray-200 p-4 md:p-6">
            <div class="flex items-center justify-between">
              <h3 class="text-lg md:text-xl font-semibold text-gray-900">Log Details</h3>
              <button onclick="document.getElementById('log-modal').remove()" class="text-gray-400 hover:text-gray-600 p-1">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-4 md:mb-6">
              <div class="space-y-3 md:space-y-4">
                ...
                <div>
                  ...
                  <div class="mt-1 font-medium text-gray-900 truncate">${log.user_email}</div>
                  <div class="text-sm text-gray-600 truncate">${log.user_id}</div>
                </div>
                <div>
              ...
              <div class="space-y-3 md:space-y-4">
                <div>
                  ...
                  <div class="mt-1">
                    ...
                    ${log.entity_name ? `<div class="text-sm text-gray-600 truncate">${log.entity_name}</div>` : ''}
                    ${log.entity_id ? `<div class="text-xs text-gray-500 truncate">ID: ${log.entity_id}</div>` : ''}
                  </div>
                </div>
                <div>
                  <label class="text-sm font-medium text-gray-500">IP Address</label>
                  <div class="mt-1 font-mono text-sm text-gray-900 truncate">${log.ip_address || 'N/A'}</div>
                </div>
                <div>
                  <label class="text-sm font-medium text-gray-500">User Agent</label>
                  <div class="mt-1 text-sm text-gray-900 truncate max-h-20 overflow-y-auto">${log.user_agent || 'N/A'}</div>
                </div>
              </div>
            </div>
            <div>
              <label class="text-sm font-medium text-gray-500 mb-2 block">Details</label>
              <div class="bg-gray-50 p-3 md:p-4 rounded-lg border border-gray-200 max-h-60 overflow-y-auto">
                <pre class="whitespace-pre-wrap text-sm font-mono text-gray-800">${details}</pre>
              </div>
            </div>
          </div>
        </div>
  ...
  const Card = ({ children, className = '' }: any) => (
    <div className={`bg-white rounded-lg md:rounded-xl border border-gray-200 shadow-sm ${className}`}>
      {children}
    </div>
  )
  ...
  const CardTitle = ({ children, className = '' }: any) => (
    <h3 className={`text-base md:text-lg font-semibold text-gray-900 ${className}`}>{children}</h3>
  )
        ...
        <div className="max-w-md w-full text-center">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                onClick={() => router.push('/administrator/dashboard')}
        ...
        <div className="max-w-7xl mx-auto space-y-4 md:space-y-6">
            ...
            <div>
              <div className="flex items-center gap-2 md:gap-3 mb-1 md:mb-2">
                <div className="p-1.5 md:p-2 bg-purple-100 rounded-lg">
                  <History className="h-6 w-6 md:h-8 md:w-8 text-purple-600" />
                </div>
                <div>
                  <h1 className="text-xl md:text-2xl lg:text-3xl font-bold text-gray-900">Audit Logs</h1>
              ...
              <div className="flex flex-wrap items-center gap-1.5 md:gap-2 mt-1 md:mt-2">
                <span className="inline-flex items-center gap-1 bg-purple-100 text-purple-800 px-2 py-0.5 md:px-3 md:py-1 rounded-full text-xs font-medium">
                  <Shield className="h-3 w-3" />
                  Super Administrator
                </span>
                <span className="inline-flex items-center gap-1 bg-blue-100 text-blue-800 px-2 py-0.5 md:px-3 md:py-1 rounded-full text-xs font-medium">
                  <BarChart3 className="h-3 w-3" />
                  {stats.total.toLocaleString()} Logs
                </span>
                {currentUser && (
          ...
          <div className="md:hidden">
            ...
            >
              ...
              {search || actionFilter !== 'all' || entityFilter !== 'all' || dateRange !== '7days' ? (
            ...
            <CardContent>
              <form onSubmit={handleSearch} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3 md:gap-4">
                  ...
                  <div className="space-y-1.5 md:space-y-2">
                    <label className="text-xs md:text-sm font-medium text-gray-700">Action Type</label>
                    <Select 
                      value={actionFilter}
                  ...
                  <div className="space-y-1.5 md:space-y-2">
                    <label className="text-xs md:text-sm font-medium text-gray-700">Entity Type</label>
                    <Select 
                      value={entityFilter}
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2 md:gap-3">
              ...
              <CardContent className="p-3 md:p-4 lg:pt-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="p-1.5 md:p-2 bg-blue-100 rounded-lg">
                    <History className="h-4 w-4 md:h-5 md:w-5 lg:h-6 lg:w-6 text-blue-600" />
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-3 md:p-4 lg:pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-lg md:text-xl lg:text-2xl font-bold text-gray-900">{stats.today}</p>
                  </div>
                  <div className="p-1.5 md:p-2 bg-green-100 rounded-lg">
                    <Calendar className="h-4 w-4 md:h-5 md:w-5 lg:h-6 lg:w-6 text-green-600" />
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-3 md:p-4 lg:pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs md:text-sm text-gray-500">Unique Users</p>
                    <p className="text-lg md:text-xl lg:text-2xl font-bold text-gray-900">{stats.uniqueUsers}</p>
                  </div>
                  <div className="p-1.5 md:p-2 bg-purple-100 rounded-lg">
                    <Users className="h-4 w-4 md:h-5 md:w-5 lg:h-6 lg:w-6 text-purple-600" />
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-3 md:p-4 lg:pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm md:text-base lg:text-lg font-bold text-gray-900 truncate capitalize" title={stats.topAction.replace('_', ' ')}>
              ...
              <CardContent className="p-3 md:p-4 lg:pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm md:text-base lg:text-lg font-bold text-gray-900 truncate capitalize" title={stats.topEntity.replace('_', ' ')}>
                      {stats.topEntity.replace('_', ' ')}
                    </p>
                  </div>
                  <div className="p-1.5 md:p-2 bg-indigo-100 rounded-lg">
                    <Database className="h-4 w-4 md:h-5 md:w-5 lg:h-6 lg:w-6 text-indigo-600" />
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-3">
                  {Object.entries(stats.actionsByType)
                    ...
                    .map(([action, count]) => (
                      <div key={action} className="bg-gray-50 p-3 rounded-lg border border-gray-200">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 md:gap-4">
                <div>
                  ...
                  <CardDescription className="text-xs md:text-sm">
                    ...
                    <span className="hidden md:inline"> (Page {pagination.page} of {pagination.totalPages})</span>
                  </CardDescription>
                </div>
                <div className="text-xs md:text-sm text-gray-600">
                  <span className="md:hidden">Page {pagination.page}/{pagination.totalPages}</span>
                  <span className="hidden md:inline">Last updated: {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
              </div>
            </CardHeader>
            <CardContent>
              {loading ? (
                <div className="flex flex-col items-center justify-center py-8 md:py-12">
                  ...
                  <p className="text-sm md:text-base text-gray-600">Loading task logs...</p>
                </div>
              ) : logs.length === 0 ? (
                <div className="text-center py-8 md:py-12">
                  <FileText className="h-8 w-8 md:h-12 md:w-12 mx-auto text-gray-400 mb-3 md:mb-4" />
                  <h3 className="text-base md:text-lg font-semibold text-gray-900 mb-1 md:mb-2">No task logs found</h3>
                    ...
                    {logs.map((log) => {
                      ...
                      return (
                        <div key={log.id} className="bg-white border border-gray-200 rounded-lg p-3 shadow-sm">
                          ...
                          <div className="mb-2">
                            <div className="text-xs font-medium text-gray-700 truncate" title={log.user_email}>
                              {log.user_email}
                            </div>
                            {log.entity_name && (
                              <div className="text-xs text-gray-600 truncate" title={log.entity_name}>
                                {log.entity_name}
                              </div>
                            )}
                          </div>
                        ...
                        {logs.map((log) => {
                          ...
                          return (
                            <tr key={log.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                              ...
                              <td className="py-3 px-4">
                                <div className="font-medium text-gray-900 text-sm truncate max-w-[180px]" title={log.user_email}>
                                  {log.user_email}
                                </div>
                                <div className="text-xs text-gray-500 truncate max-w-[180px]" title={log.user_id}>
                              ...
                              <td className="py-3 px-4">
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    ...
                                    {log.entity_name && (
                                      <div className="text-xs text-gray-500 truncate max-w-[150px]" title={log.entity_name}>
                                        {log.entity_name}
                                      </div>
                                    )}
                                  </div>
                          ...
                          {Array.from({ length: Math.min(isMobile ? 3 : 5, pagination.totalPages) }, (_, i) => {
                            ...
                            return (
                              <Button
                                ...
                                className="px-2 md:px-3 py-1 md:py-1.5 min-w-[32px] md:min-w-[40px] text-xs md:text-sm"
                                disabled={loading}
                              >
                                {pageNum}
                              </Button>
          ...
          <Card>
            <CardContent className="p-4 md:p-6">
              <div className="flex items-start gap-2 md:gap-3">
                <AlertCircle className="h-4 w-4 md:h-5 md:w-5 text-blue-500 mt-0.5 flex-shrink-0" />
                <div className="space-y-3 md:space-y-4">
                  <div>
                    <h4 className="text-sm md:text-base font-medium text-gray-900 mb-1">About Audit Logs</h4>
                    <p className="text-xs md:text-sm text-gray-600">
