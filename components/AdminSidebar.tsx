...
export default function AdminSidebar({
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-40 w-72 transform border-r border-blue-800 bg-[#0b1b3b] text-white transition-transform duration-300 ease-in-out md:static md:translate-x-0',
      ...
      >
        ...
        <button
          onClick={() => setOpen(false)}
          className="absolute top-4 right-4 p-2 rounded-lg bg-blue-700 hover:bg-blue-600 transition-colors md:hidden"
        >
                ...
                <li key={href}>
                  <Link
                    ...
                    className={cn(
                      ...
                      active ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-200'
                    )}
                    onClick={() => setOpen(false)}
                  >
...
export function MobileTopbar({ onMenu }: { onMenu: () => void }) {
  ...
  return (
    <div className="sticky top-0 z-30 flex items-center gap-3 border-b border-blue-800 bg-[#0b1b3b] px-4 py-3 text-white md:hidden">
      <button
        onClick={onMenu}
        className="grid size-9 place-items-center rounded-lg border border-blue-700 bg-[#11214a] hover:bg-blue-700 transition-colors"
      >
